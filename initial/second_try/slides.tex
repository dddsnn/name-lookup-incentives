...
\begin{frame}{Reputation Tracking}
  \begin{itemize}
    \item Two kinds of groups
    \begin{itemize}
      \item sync groups: necessary for DHT, updates for own key space
      \item query groups: small groups in which queries are sent
    \end{itemize}
    \item Reputation tracked in query groups (locally)
...
\begin{frame}{Goal of the Thesis}
  \begin{itemize}
    \item Does such an incentive system lead to a functioning system?
    \item Does every well-behaving peer have enough reputation to receive good
          (non-penalized) service?
